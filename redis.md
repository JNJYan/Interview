...
rehash时，要先确定新哈希表的大小，有两种选择方法：2的幂和素数。
- 2的幂：位运算取余，速度提升
- 素数：得到最为分散的余数，减少哈希冲突
### rehash触发
负载因子计算公式
...
在执行`BGSAVE`或`BGREWRITEAOF`过程中，Redis需要创建当前服务器进程的子进程，而大多数操作系统采用CoW(copy on write)来优化子进程的使用效率，所以在进程存在期间，服务器会提高扩展操作所需的负载因子，从而尽可能避免在子进程存在期间进行哈希表扩展操作，尽可能避免不必要的内存写入操作，最大限度地节约内存。
